  ...
  while (true) {
    ...
    try {
      ...
      if (jogoExist.length) {
        ...
        const day = (dataJogo.getDate() + 2) % 30
        const month = day === 0 ? (dataJogo.getMonth() + 1) % 11 : dataJogo.getMonth()
        const jogo2Dias = await db.findByTimeExact(table, hora, minuto, day, month)
        if (jogo2Dias) {
          // jogo de ontem
          console.log(`${color[campeonato]}(${campeonato}) ATUALIZAR Jogo `)

          // await db.update(table, jogo)
          await db.remove(table, jogo2Dias)
