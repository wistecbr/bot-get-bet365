...
const run = async (campeonato, nomeCampeonato) => {
  return new Promise(async (resolve) => {
    const dbErros = new Db(`./data/error/${campeonato}.json`, { erros: [] })
    const tableErros = 'erros'
    try {
      const hoje = new Date()
      console.log(`(${campeonato}) Carregando Banco de dados ...`)
      const db = new Db(`./data/${campeonato}.json`, { jogos: [] })
      const table = 'jogos'
      ...
      // console.log('>> ', placar, placarCasa, placarFora)
      const jogo = {
        id: Date.now(),
        hora,
        minuto,
      ...
      // console.log('>> ', all)
      // const t = await db.findById(table, 1688091784416)
